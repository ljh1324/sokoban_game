...
class BoardPainter:
  def __init__(self, board_rect, table_shape):
    ...
    self.__PORTAL_COLOR = {}
    self.__PORTAL_COLOR[RED_PORTAL] = RED
    self.__PORTAL_COLOR[BLUE_PORTAL] = BLUE
    self.__PORTAL_COLOR[YELLOW_PORTAL] = YELLOW
  ...
  def draw_board(self, screen, board):  # 게임판을 그려줍니다.
    for i in range(self.__table_height):   # screen의 canvas가 나타내는 부분을 TILE_IMAGE로 덮어줍니다.
      for j in range(self.__table_width): 
        ...
        image_x = self.__canvas_x + self.__delta_x * j
        image_y = self.__canvas_y + self.__delta_y * i
        screen.blit(self.__TILE_IMAGE, (image_x, image_y, self.__delta_x, self.__delta_y))
    ...
    for i in range(self.__table_height):
      for j in range(self.__table_width):  # board[i][j]의 값에 따라 screen에 이미지를 그려줍니다.
        ...
        image_y = self.__canvas_y + self.__delta_y * i
        if board[i][j] == BRICK:
          screen.blit(self.__BRICK_IMAGE, (image_x, image_y, self.__delta_x, self.__delta_y))
        elif board[i][j] == RED_STONE:
          screen.blit(self.__RED_STONE_IMAGE, (image_x, image_y, self.__delta_x, self.__delta_y))
        elif board[i][j] == BLUE_STONE:
          screen.blit(self.__BLUE_STONE_IMAGE, (image_x, image_y, self.__delta_x, self.__delta_y))
        elif board[i][j] == YELLOW_STONE:
          screen.blit(self.__YELLOW_STONE_IMAGE, (image_x, image_y, self.__delta_x, self.__delta_y))
        elif board[i][j] == PLAYER:
          screen.blit(self.__PLAYER_IMAGE, (image_x, image_y, self.__delta_x, self.__delta_y))
        elif utils.is_portal(board[i][j]):  # portal일 경우 portal 위치에 원을 그려줍니다. 
          color = self.__PORTAL_COLOR[board[i][j]]
          pygame.draw.ellipse(screen, color, (image_x, image_y, self.__delta_x, self.__delta_y), 3)
  ...
  def draw_portal(self, screen, portal_list):  # 포탈을 그려줍니다.
    for portal in portal_list:
      ...
      y = portal[1]
      want = portal[2]
      color = self.__PORTAL_COLOR[want]

      image_x = self.__canvas_x + self.__delta_x * x
      image_y = self.__canvas_y + self.__delta_y * y
      pygame.draw.ellipse(screen, color, (image_x, image_y, self.__delta_x, self.__delta_y), 3)